...
DATE_OLD=$(grep -e '^[%#]global date' ${SOURCE_DIR}/${REPO_NAME}.spec | awk '{ print $3 }')
...
sed -e '/^%global gitcommit/ { s/^#/%/; s/'${HASH_OLD}'/'${HASH_NEW}'/ }' \
    -e '/^%global date/ s/'${DATE_OLD}'/'${DATE_NEW}'/' \
    -e "/^%changelog/ a\
* ${WEEKDAY} ${MONTH} ${DAY} ${YEAR} Yu Watanabe <watanabe.yu@gmail.com> - ${DATE_NEW}-1.git${HASH_NEW_SHORT}\\
- ${CHANGE_LOG}" \
